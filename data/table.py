...
class Chat(SqlAlchemyBase):
    __tablename__ = "chats"
    id = sqlalchemy.Column(sqlalchemy.Integer, primary_key=True, autoincrement=True)
    user1 = sqlalchemy.Column(sqlalchemy.String, sqlalchemy.ForeignKey('users.username'), unique=False, nullable=False)
    user2 = sqlalchemy.Column(sqlalchemy.String, sqlalchemy.ForeignKey('users.username'), unique=False, nullable=False)
...
class Message(SqlAlchemyBase):
    ...
    unix_time = sqlalchemy.Column(sqlalchemy.Float, unique=False, nullable=False)
    chat_id = sqlalchemy.Column(sqlalchemy.String, sqlalchemy.ForeignKey('chats.chat_id'), unique=False,
                                nullable=False)
    viewed = sqlalchemy.Column(sqlalchemy.Boolean, unique=False, nullable=False)
    sent_by = sqlalchemy.Column(sqlalchemy.String, sqlalchemy.ForeignKey('users.username'), unique=False,
                                nullable=False)
